package com.google.widget.view.reminder;
 ...
 * des ：This view should be added to WindowManager, so we can drag it to anywhere.
 * gitVersion：$Rev$
 * updateAuthor：$Author$
 * updateDate：$Date$
 * updateDes：${TODO}
...
public class GooView extends View {
	...
	float dragCircleRadius = 0;//拖拽圆半径
	float stickCircleRadius = 0;//固定圆半径
	float stickCircleMinRadius = 0;//固定圆最小半径
	float stickCircleTempRadius = stickCircleRadius;//固定圆当前半径
	float farest = 0;//断开的最大距离
...
//		path.addCircle((float)stickPoints[1].x, (float)stickPoints[1].y, 5, Direction.CW);
	...
	private float getCurrentRadius(float distance) {
		...
		return  GeometryUtil.evaluateValue(fraction, stickCircleRadius, stickCircleMinRadius);
	}
	...
	private void handleActionUp() {
		if(isOutOfRange){
			// When user1 drag it back, we should call onReset().
